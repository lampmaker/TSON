export class TSONParser {
  ...
  _processKeyValue(key, rest, indent) {
    ...
    if (!type) {
      // Check for indentation-based object grouping
      if (indent > ctx.indent) {
        // Find the most recent key that could be a parent
        const keys = Object.keys(ctx.value || {});
        if (keys.length > 0) {
          const lastKey = keys[keys.length - 1];
          // Convert the last value to an object if it's not already
          if (typeof ctx.value[lastKey] !== 'object' || ctx.value[lastKey] === null) {
            ctx.value[lastKey] = {};
          }
          // Push new context for this indented block
          this.contextStack.push({ indent, key: lastKey, value: ctx.value[lastKey] });
          // Process the current line as a key-value pair in the new context
      ...
      } else if (indent <= ctx.indent && this.contextStack.length > 1) {
